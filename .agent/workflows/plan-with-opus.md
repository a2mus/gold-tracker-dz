...
> To execute the plan, the user must manually run `/execute-with-gemini` as a separate step.

This workflow creates a detailed implementation plan for a feature or task. The plan can later be executed by the `/execute-with-gemini` workflow (The Builder) if the user chooses to run it.
...
## Execution Checklist (for /execute-with-gemini)
...
**This workflow is now complete.** Please review the plan. When you're ready to execute, run `/execute-with-gemini` separately."

> [!IMPORTANT]
> **STOP HERE**. This workflow ends after presenting the plan.
> Do NOT proceed with any implementation or execution.
> The user must manually invoke `/execute-with-gemini` if they want to execute the plan.

## Output
The plan should be saved in `memory-bank/implementation-plan.md`. Execution is **NOT** part of this workflow - it must be triggered separately by the user via `/execute-with-gemini`.
